...
type DbAuthKey = { KeyType: string; KeyCode: string }
...
module DbAuthKey =
    ...
    let toDomain v =
        if String.Equals(v.KeyType, "nfc", StringComparison.InvariantCultureIgnoreCase) then
            NFCAuthKey v.KeyCode
        else
            failwith $"Unknown key type \"{v.KeyType}\""
...
module User =
    ...
    let getByAuthKey authKey =
        ...
        DB.readSingle
            ...
            (fun reader -> {
                ...
                AuthKeys =
                    JsonSerializer.Deserialize<DbAuthKey[]>(reader.GetString(3), DbAuthKey.jsonOptions)
                    |> Seq.map DbAuthKey.toDomain
                    |> Seq.toList
                Role =
                    reader.GetString(4)
                    |> fun roleName ->
                        UserRole.tryParse roleName
                        |> Option.defaultWith (fun () -> failwithf "DB error: Can't parse user role \"%s\"" roleName)
            })

    let getByAuthKeyDto (authKey: string) =
        authKey |> Option.ofObj |> Option.bind AuthKey.tryParse |> Option.bindTask getByAuthKey
    ...
    let getBalance (UserId userId) =
        task {
            let! totalOrderPrice =
                DB.readSingle
                    "SELECT COALESCE(SUM(`amount` * (`pricePerUnit` * 100)), 0) as `price` FROM `Order` WHERE userId = @UserId"
                    [ ("@UserId", userId) ]
                    (fun reader -> reader.GetDecimal(0) / 100m)

            let! totalBalance =
                DB.readSingle
                    "SELECT COALESCE(SUM(`amount` * 100), 0) FROM `MemberPayment` WHERE userId = @UserId"
                    [ ("@UserId", userId) ]
                    (fun reader -> reader.GetDecimal(0) / 100m)
...
module Order =
    ...
    let getProductGroups () =
        task {
            let! groups =
                DB.read "SELECT `id`, `name` FROM `ArticleGroup` ORDER BY `grade`" [] (fun reader ->
            ...
            return
                ...
                |> List.choose (fun (groupId, groupName) ->
                    ...
                    match products with
                    | [] -> None
                    | x -> Some { Name = groupName; Products = x })
        }
    ...
    let saveOrder data (userId: UserId) =
        task {
            let! productData =
                task {
                    ...
                    let parameterNames =
                        productIds
                        |> Seq.mapi (fun i _ -> sprintf "@ArticleId%d" i)
                        |> String.concat ", "

                    let query =
                        sprintf "SELECT `id`, `name`, `price` FROM `Article` WHERE `id` IN (%s)" parameterNames
                    ...
                    return!
                        DB.readIndexed query parameters (fun reader ->
            ...
            | Ok parameters ->
                do!
                    DB.writeMany
                        "INSERT INTO `Order` (`id`, `userId`, `articleName`, `amount`, `pricePerUnit`, `timestamp`) VALUES (@Id, @UserId, @ArticleName, @Amount, @PricePerUnit, @Timestamp)"
                        parameters
...
module UserPaymentAdministration =
    ...
    let savePayment (UserId userId) (amount: decimal) =
        task {
            let parameters = [
                ("@Id", Guid.NewGuid() |> Helper.Box)
                ("@UserId", Helper.Box userId)
...
module UserAdministration =
                        ...
                        Data = {
                            FirstName =
                                ...
                                |> Option.defaultWith (fun () ->
                                    failwithf "DB error: First name of user (%O) is empty" userId)
                            LastName =
                                reader.GetString(2)
                                |> NotEmptyString.tryCreate
                                |> Option.defaultWith (fun () ->
                                    failwithf "DB error: Last name of user (%O) is empty" userId)
                            AuthKeys =
                                JsonSerializer.Deserialize<DbAuthKey[]>(reader.GetString(3), DbAuthKey.jsonOptions)
                                |> Seq.map DbAuthKey.toDomain
                                |> Seq.toList
                            Role =
                                ...
                                |> fun roleName ->
                                    UserRole.tryParse roleName
                                    |> Option.defaultWith (fun () ->
                                        failwithf "DB error: Can't parse user role \"%s\"" roleName)
                        }
                    })
        }
    ...
    let createUser (data: ExistingUserData) =
        task {
            try
                ...
                return Ok newUserId
            with :? SqliteException as e when isDuplicateAuthKeyException e ->
                let! users =
                    data.AuthKeys
                    |> List.map User.getByAuthKey
                    |> System.Threading.Tasks.Task.WhenAll

                let userNames =
                    users
                    |> Seq.choose (Option.map (fun v -> $"{v.LastName} {v.FirstName}"))
                    |> Seq.toList

                return Error(KeyCodeTaken userNames)
        }

    let updateUser (userId: UserId) (data: PatchUserData) =
        task {
            try
                ...
                let updateFields =
                    fields |> Seq.map (fst >> fun v -> $"`%s{v}`=@%s{v}") |> String.concat ", "
            ...
            return!
                DB.executeCommands [
                    ("UPDATE `Member` SET `deleteTimestamp` = @DeleteTimestamp WHERE `id` = @Id",
                     [
                         ("@Id", Helper.Box userId)
                         ("@DeleteTimestamp", Helper.Box DateTimeOffset.Now)
                     ])
                    ("DELETE FROM `AuthKey` WHERE `userId`=@Id", [ ("@Id", Helper.Box userId) ])
                ]
        }
...
module ProductAdministration =
...

                    {
                        ...
                        Data = {
                            Name =
                                ...
                                |> NotEmptyString.tryCreate
                                |> Option.defaultWith (fun () ->
                                    failwithf "DB error: Name of product group (%O) is empty" productGroupId)
                        }
                        Products = []
                    })
                        ...
                        let product = {
                            ...
                            Data = {
                                Name =
                                    ...
                                    |> NotEmptyString.tryCreate
                                    |> Option.defaultWith (fun () ->
                                        failwithf "DB error: Name of product (%O) is empty" productId)
                                Price =
                                    reader.GetDecimal(3)
                                    |> NonNegativeDecimal.tryCreate
                                    |> Option.defaultWith (fun () ->
                                        failwithf "DB error: Price of product (%O) is invalid" productId)
                                State =
                                    let state = reader.GetString(4)

                                    ProductState.tryParse state
                                    |> Option.defaultWith (fun () ->
                                        failwithf "DB error: Unknown product state (\"%s\")" state)
                            }
                        }
        ...
        task {
            ...
            do!
                DB.write
                    ...
                    [ "@Id", Helper.Box newProductGroupId; "@Name", Helper.Box data.Name ]

            return newProductGroupId
        }

    let updateProductGroup (productGroupId: ProductGroupId) data =
        DB.write "UPDATE `ArticleGroup` SET `name` = @Name Where `id` = @Id" [
    ...
    let createProduct (productGroupId: ProductGroupId) (data: ProductData) =
        task {
    ...
    let updateProduct (productId: ProductId) data =
        DB.write "UPDATE `Article` SET `state` = @State, `name` = @Name, `price` = @Price Where `id` = @Id" [
...
module OrderAdministration =
    ...
    let getOrders () =
        ...
        """
            ...
            return!
                DB.read query [ ("@OldestTime", DateTimeOffset.Now.AddMonths(-1) |> Helper.Box) ] (fun reader -> {
    ...
    let deleteOrder (orderId: OrderId) =
        task { do! DB.write "DELETE FROM `Order` WHERE `id` = @Id" [ ("@Id", Helper.Box orderId) ] }

module OrderStatistics =
    open MusiOrder.Models.OrderStatistics

    let getOrders (startTime: DateTime) (endTime: DateTime) =
        task {
            let query =
                """
            SELECT `O`.`id`, `M`.`firstName`, `M`.`lastName`, `O`.`articleName`, `O`.`amount`, `O`.`pricePerUnit`, `O`.`timestamp`
        ...
        """
            ...
            return!
                DB.read query [ "@OldestTime", Helper.Box startTime; "@NewestTime", Helper.Box endTime ] (fun reader -> {
                    FirstName = reader.GetString(1)
                    LastName = reader.GetString(2)
                    ProductName = reader.GetString(3)
                    Amount = reader.GetInt32(4)
