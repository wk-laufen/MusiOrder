...
let initTestDb () =
    let dbPath =
        Path.Combine(Path.GetTempPath(), $"musiorder-test-{Guid.NewGuid()}.sqlite")
...
let createTestClient (authHandler: IAuthHandler) =
    task {
        let host =
            (new HostBuilder())
                .ConfigureWebHost(fun webBuilder ->
                    webBuilder
                        ...
                        .ConfigureServices(fun services ->
                            services
                                .AddControllers()
                                .AddApplicationPart(typeof<MusiOrder.Server.Controllers.OrderController>.Assembly)
                                .AddJsonOptions(fun opts ->
                                    ...
                                    opts.JsonSerializerOptions.NumberHandling <-
                                        Serialization.JsonNumberHandling.AllowReadingFromString)
                            |> ignore
...
let seedMember (id: string) (firstName: string) (lastName: string) (role: string) =
    DB.write "INSERT INTO `Member` (`id`, `firstName`, `lastName`, `role`) VALUES (@Id, @FirstName, @LastName, @Role)" [
        ("@Id", box id)
...
let seedAuthKey (keyCode: string) (userId: string) =
    DB.write
        ...
        [
            ("@KeyCode", box keyCode)
            ("@UserId", box userId)
            ("@CreationTime", box (DateTimeOffset.Now.ToString("o")))
        ]
...
let seedProductGroup (name: string) =
    task {
        let id = sprintf "%O" (Guid.NewGuid())
...
let seedProduct (groupId: string) (name: string) (price: decimal) =
    task {
        let id = sprintf "%O" (Guid.NewGuid())
...
let seedOrder (userId: string) (productName: string) (amount: int) (price: decimal) =
    task {
        let id = sprintf "%O" (Guid.NewGuid())
...
let seedPayment (userId: string) (amount: decimal) =
    task {
        let id = sprintf "%O" (Guid.NewGuid())
...
let authQuery (key: string) =
    sprintf "?authKey=%s" (System.Uri.EscapeDataString(key: string))
