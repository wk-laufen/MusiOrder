    ...
    testList "UserPaymentAdministration" [
        testList "getUsers" [
            ...
            testTask "unknown-format authKey returns 400 InvalidAuthKey" {
                ...
                let! (response: HttpResponseMessage) =
                    client.GetAsync("/api/administration/user-payment/users?authKey=notanfc")
            ...
            testTask "valid-format but unknown authKey returns 400 InvalidAuthKey" {
                ...
                let! (response: HttpResponseMessage) =
                    client.GetAsync("/api/administration/user-payment/users?authKey=nfc/unknownkey")
            ...
            testTask "non-admin authKey returns 400 NotAuthorized" {
                ...
                let! (response: HttpResponseMessage) =
                    client.GetAsync($"/api/administration/user-payment/users{authQuery userKey}")
            ...
            testTask "admin authKey returns 200 with user list" {
                ...
                let! (response: HttpResponseMessage) =
                    client.GetAsync($"/api/administration/user-payment/users{authQuery adminKey}")
        ...
        testList "postPayment" [
            testTask "missing authKey returns 400 InvalidAuthKey" {
                ...
                let! (response: HttpResponseMessage) =
                    client.PostAsync($"/api/administration/user-payment/{userId}", jsonContent """{"amount":5.0}""")
            ...
            testTask "non-admin authKey returns 400 NotAuthorized" {
                ...
                let! (response: HttpResponseMessage) =
                    client.PostAsync(
                        $"/api/administration/user-payment/{userId}{authQuery userKey}",
                        jsonContent """{"amount":5.0}"""
                    )
            ...
            testTask "admin authKey with valid amount returns 200 with new balance" {
                ...
                let! (response: HttpResponseMessage) =
                    client.PostAsync(
                        $"/api/administration/user-payment/{userId}{authQuery adminKey}",
                        jsonContent """{"amount":10.0}"""
                    )

                Expect.equal response.StatusCode HttpStatusCode.OK "should return 200"
                let! balance = response.Content.ReadFromJsonAsync<decimal>()
            ...
            testTask "admin authKey, user with existing order returns correct balance" {
                ...
                do!
                    seedOrder userId "TestBeer" 2 3.5m
                    |> (fun (t: System.Threading.Tasks.Task<string>) -> t :> System.Threading.Tasks.Task)
